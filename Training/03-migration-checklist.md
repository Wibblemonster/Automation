...
> **Golden rule:** POM _holds_ the tools ·· Fixture _builds_ the tools ·· Test _uses_ the tools.

**What that actually means** (think of a workshop 🔧):

- **POM = the toolbox.** Each `pages/*.ts` file _holds_ the tools for one page — the **locators**
...
- **Fixture = the assistant who hands you the toolbox.** `fixtures/test-options.ts` _builds_ the
...
- **Test = the job you're doing.** `first-test.spec.ts` _uses_ the tools — it calls the actions
